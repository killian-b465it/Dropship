...
const CONNECTED_KEY = 'dropship_connected_suppliers';

function getConnected() {
    try { return JSON.parse(localStorage.getItem(CONNECTED_KEY)) || []; }
    catch { return []; }
}

function toggleConnect(supplierId) {
    const connected = getConnected();
    const idx = connected.indexOf(supplierId);
    if (idx > -1) {
...
function openConnectModal(supplierId) {
    const s = SUPPLIERS_DATA.find(x => x.id === supplierId);
    if (!s) return;
    const connected = getConnected();
    const isConnected = connected.includes(supplierId);
